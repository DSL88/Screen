...
function isBullishState(s) {
  // Regimes Bull: currentState == 5 or currentState == 7
  return s === 5 || s === 7;
}

function isBearishState(s) {
  // Regimes Bear: currentState == 3 or currentState == 6
  return s === 3 || s === 6;
}

function buildStateSeries(bbPct, rsi, adx) {
  // Note: rsi is accepted for signature compatibility but not used in the 2D classification
...
function buildTransitionMatrix(states, window) {
  const alpha = 0.1; // Laplace smoothing coefficient
...
function analyzeSeries(candles, params = {}) {
  ...
  const volumeValid = !useVolFilter || (lastVol != null && lastVolSma != null && lastVol > lastVolSma * volThresh);
